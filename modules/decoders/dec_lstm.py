...
class LSTMDecoder(DecoderBase):
    ...
    def __init__(self, args, vocab, model_init, emb_init, encode_length):
        ...
        self.trans_linear = nn.Linear(self.nz, args.dec_nh, bias=False)


        self.lstm = nn.LSTM(input_size=args.ni + self.nz,
        ...
        # prediction layer
        self.pred_linear = nn.Linear(args.dec_nh, len(vocab), bias=pred_linear_bias)
    ...
    def decode(self, input, z):
        ...
        if len(z.size()) == 3:
            ...
            z = z[:,:,1:,:] # cut last seq_len
            z_ = z.view(batch_size * n_sample, seq_len, self.nz)
            # Prepare z for computing hidden state by taking the
            # first position z.
            z = z[:,:,0,:]
    ...
    def beam_search_decode(self, z, K=5, no_unk=False):
        ...
        batch_size = z.size(0)
        nz = z.size(-1)
        pos_specific_z = (len(z.size()) == 3)
        if pos_specific_z:
            z_ = z[:,1] # that's what we use in self.decode()
        else:
            z_ = z
            #z_ = z[:,0]

        # (1, batch_size, nz)
        c_init = self.trans_linear(z_).unsqueeze(0)
        h_init = torch.tanh(c_init)
        ...
        for idx in range(batch_size):
            ...
            t = 0
            while len(completed_hypotheses) < K and t < max_len:
                t += 1
                ...
                if pos_specific_z:
                    ...
                    z_ = z
                word_embed = torch.cat((word_embed, z_[idx].view(1, 1, -1).expand(
                ...
                output_logits = self.pred_linear(output)
                if no_unk:
                    self.logits_unk_infinite(output_logits)
    ...
    def logits_unk_infinite(self, logits):
        """ Inplace set UNK symbol to -inf (to disable sampling of UNKs)
        """
        inf = - float('inf')
        ...
        if len(logits.size()) == 2:
            ...
            logits[:,:,unk_id] = inf
        elif len(logits.size()) == 4:
            logits[:,:,:,unk_id] = inf
        else:
            raise NotImplementedError()
        #logits.index_select(-1, torch.Tensor([]))

    def greedy_decode(self, z, no_unk):
        return self.sample_decode(z, greedy=True, no_unk=no_unk)

    def sample_decode(self, z, greedy=False, no_unk=False):
        """sample/greedy decoding from z
        Args:
            z: (batch_size, nz) or (batch_size, n_words, nz)
        ...
        while mask.sum().item() != 0 and length_c <= max_len:
            ...
            if pos_specific_z:
                ...
                z_ = z
            word_embed = torch.cat((word_embed, z_.unsqueeze(1)), dim=-1)
