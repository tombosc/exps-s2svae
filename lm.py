...
def init_config():
    ...
    parser.add_argument('--eval', action='store_true', default=False,
                        help='compute iw nll')
    parser.add_argument('--export-avg-loss-per-ts', default=None,
                        help='Export average loss per timestep')
    ...
    # others
    parser.add_argument('--seed', type=int, default=783435, metavar='S',
                        help='random seed')
...
def main(args):
    global logging
    debug = (args.eval == True or args.export_avg_loss_per_ts) # don't make exp dir 
    logging = create_exp_dir(args.exp_dir, scripts_to_save=["lm.py"], debug=debug)
    ...
    val_data = MonoTextData(args.val_data, vocab=vocab, label=args.label)
    test_data = MonoTextData(args.test_data, vocab=vocab, label=args.label)
    ...
    if not hasattr(args, 'log_niter'):
        args.log_niter = (len(train_data)//args.batch_size)//10
